...
### Logs & Export
- **Ruedi Data** tab: filter by project, export filtered records as combined CSV or multi-page PDF field sheets
...
| File | Purpose |
|------|---------|
| `ruedi-field.html` | Main application — single file, zero dependencies |
| `ruedi-manifest.json` | PWA manifest — name, icons, shortcuts |
| `ruedi-service-worker.js` | Offline cache |
| `ruedi-icon-192.png` | App icon 192×192 |
| `ruedi-icon-192-maskable.png` | Android adaptive icon 192×192 |
| `ruedi-icon-512.png` | App icon 512×512 |
| `ruedi-icon-512-maskable.png` | Android adaptive icon 512×512 |
| `ruedi-icon-1024.png` | High-res icon for App Store/Play Store |
| `ruedi-icon-180.png` | Apple Touch Icon |
| `ruedi-icon-96.png` | Favicon |

---

## Deployment — GitHub Pages

1. Create a new GitHub repository (e.g. `RuediField`)
2. Upload all files from the zip
3. Go to **Settings → Pages → Source** → branch: `main`, folder: `/ (root)` → Save
4. App live at: `https://yourusername.github.io/RuediField/ruedi-field.html`
...
4. Bump `CACHE_VERSION` in `ruedi-service-worker.js` (e.g. `v1` → `v2`) — this forces installed apps to fetch the new version
...
*Ruedi Field · MiniRUEDI Gas Analysis & GW Monitoring*
